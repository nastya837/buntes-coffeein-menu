...
| **Графики** 📊 | Круговая диаграмма расходов по категориям и сравнение с прошлым месяцем — картинкой прямо в чате (кнопки под `/report`). |
| **Советы** 💡 | Спроси «где я перетрачиваю?», «как накопить на отпуск?» — ответит с учётом твоих данных *(нужен AI-ключ)*. |
| **Обязательные платежи** 🔔 | `/remind` — раз в неделю / месяц / год (аренда, подписки, страховки). Бот напомнит в нужный день. |
